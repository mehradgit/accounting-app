...
async function createAccountingVoucher(
  ...
  console.log(
    `💰 شروع ایجاد سند حسابداری برای خرید - روش پرداخت: ${paymentMethod}`
  );

  // ایجاد شماره سند (می‌توان این را ابتدا گرفت)
  const lastVoucher = await tx.voucher.findFirst({ orderBy: { id: "desc" } });
  const voucherNumber = generateVoucherNumber(lastVoucher?.id || 0);

  // =================================================================
  // 🔍 ۱. پیدا کردن اطلاعات حساب‌ها — اینها باید قبل از ساخت description/voucher باشند
  // =================================================================
  // حساب هزینه/خرید
  ...
  if (expenseDetailAccountId) {
    ...
    if (expenseDetailAccount) {
      ...
      console.log(
        `💰 حساب خرید: ${expenseDetailAccount.name} (${expenseSubAccount.code})`
      );
    }
  }
  ...
  if (supplierDetailAccountId) {
    ...
    if (supplierDetailAccount) {
      ...
      console.log(
        `🏢 تامین‌کننده: ${supplierDetailAccount.name} (${supplierSubAccount.code})`
      );
    }
  }
  ...
  if (bankDetailAccountId) {
    ...
    if (bankDetailAccount) {
      ...
      console.log(
        `🏦 حساب بانک: ${bankDetailAccount.name} (${bankSubAccount.code})`
      );
    }
  }

  // =================================================================
  // اکنون که حساب‌ها در دسترس هستند، می‌توان description را ساخت و voucher ایجاد کرد
  // =================================================================
  const voucherDescription = `خرید مواد اولیه از ${
    supplierDetailAccount?.name || "تامین‌کننده"
  } - سند انبار ${document.documentNumber}${
  ...
  // ادامهٔ منطق (ایجاد آیتم‌های سند، آپدیت مانده‌ها و ...)
  // ... (سایر بخش‌های تابع بدون تغییر)
  // در کد شما ادامهٔ تابع همانند قبل باید باقی بماند؛ فقط ترتیب ایجاد voucher و lookup حساب‌ها اصلاح شد.

  // سپس بسته به روش پرداخت، فراخوانی createCreditPurchaseVoucherItems یا createCashPurchaseVoucherItems و updateAccountBalances
...
async function createCashPurchaseVoucherItems(
  ...
  console.log(
    `💰 ایجاد سند ۴ ردیفی برای خرید ${getPaymentMethodLabel(paymentMethod)}`
  );
  ...
  if (supplierDetailAccount && supplierSubAccount) {
    const debitItemData = {
      ...
      subAccountId: supplierSubAccount.id,
      detailAccountId: supplierDetailAccount.id,
      description: `۳. بدهکار: تسویه بدهی ${
        supplierDetailAccount.name
      } با ${getPaymentMethodLabel(paymentMethod)}`,
      debit: totalAmount,
      credit: 0,
    };
...
async function createPaymentCreditLine(
  ...
  switch (paymentMethod) {
    case "cash": // پرداخت نقدی
      ...
      const cashAccount = await tx.subAccount.findFirst({
        where: {
          OR: [{ code: "1-01-0002" }, { name: { contains: "صندوق" } }],
        },
      });
      ...
      if (cashAccount) {
        ...
        console.log(
          `💰 پیدا کردن حساب صندوق: ${cashAccount.code} - ${cashAccount.name}`
        );
      }
      break;
    ...
    case "transfer": // پرداخت حواله
      if (bankDetailAccount) {
        ...
        console.log(
          `🏦 پیدا کردن حساب بانک: ${bankDetailAccount.subAccount.code} - ${bankDetailAccount.name}`
        );
      }
      break;
    ...
    case "cheque": // پرداخت چکی
      ...
      if (chequesPayableAccount) {
        ...
        console.log(
          `🧾 پیدا کردن حساب چک‌های پرداختنی: ${chequesPayableAccount.code} - ${chequesPayableAccount.name}`
        );
      }
      break;
  ...
  if (subAccountId) {
    ...
    console.log(
      `📝 ردیف ۴ - بستانکار ${getPaymentMethodLabel(
        paymentMethod
      )}: ${totalAmount} ریال`
    );
  } else {
...
async function updateAccountBalances(
  ...
  if (paymentMethod === "credit") {
    ...
    if (supplierSubAccountId) {
      ...
      console.log(
        `📈 مانده حساب تامین‌کننده افزایش یافت (بدهی نسیه): ${totalAmount} ریال`
      );
    }
  } else {
    ...
    switch (paymentMethod) {
      ...
      case "cheque": // چک‌های پرداختنی
        ...
        if (chequesPayableAccount) {
          ...
          console.log(
            `📈 مانده حساب چک‌های پرداختنی افزایش یافت: ${totalAmount} ریال`
          );
        }
        break;
    }
...
async function createPurchaseCheque(
  ...
  try {
    ...
    // نام تامین‌کننده
    const payee =
      supplierDetailAccount?.person?.name ||
      supplierDetailAccount?.name ||
      "تامین‌کننده";
    ...
    const cheque = await tx.cheque.create({
      data: {
        ...
        // ارتباط‌ها
        personId: personId,
        bankDetailAccountId: bankDetailAccountId
          ? parseInt(bankDetailAccountId)
          : null,
        expenseDetailAccountId: expenseDetailAccountId
          ? parseInt(expenseDetailAccountId)
          : null,
        payeeDetailAccountId: supplierDetailAccountId
          ? parseInt(supplierDetailAccountId)
          : null,
        voucherId: voucherId,
      },
    });
